...
import sys
#index = sys.argv[1] ##This is used if running multiple parallel codes
index = 1
...
def simulation(g, m, lamb, k, alpha, beta, nu, e, tau1, tau2,f):
    ...
    #initial conitions for spore, newly germinated bacteria, vegetative bacteria and nutrients
    ...
    spore_sim_full = R[:,0] + (1-f)*S0 #model spore count, plus the number of spores we estimate will not germinate
    NGB_sim_full = R[:,1]
    veg_sim_full = R[:,2] 
    nut_sim_full = R[:,3]
    ...
    #Add random noise to the simulations    
...
def other_iterations(n, eps1, eps2):
   ...
   #Since we use a uniform kernel, here we computing the ranges of each of the
   ...
   while accepted < n:
      ...
      if ((model[0] > 0).sum() == model[0].size).astype(int)==1:# and ((model[1] >= 0).sum() == model[1].size).astype(int)==1 and ((model[2] >= 0).sum() == model[2].size).astype(int)==1:
          ...
          if dist1 < eps1 and dist2 < eps2:
             ...
             #Compute the final weight. Since we have uniform priors, the numerators
             #will cancel in the normalisation of the weights
...
#define sample size and number of iterations
sample_size = 200
num_iters = 20

#Generate first value of epsilon by getting the distance for 5 random parameter sets
...
#Run first round of ABC with generated epsilons
...
for k in range(num_iters):
   ...
   #can be useful if one population converges quicker than the other
   '''
   ...
   #save the results and the epsilons
   ...
   #save the distances for each parameter set and the parameter sets themselves
